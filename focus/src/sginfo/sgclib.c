...
static void SwapRotMxInfo(T_RotMxInfo *RMx_a, T_RotMxInfo *RMx_b)
{
  ...
  (void) memcpy(&Buffer, RMx_a,   sizeof (Buffer));
  (void) memcpy(RMx_a,   RMx_b,   sizeof (Buffer));
  (void) memcpy(RMx_b,   &Buffer, sizeof (Buffer));
}
  ...
  if (ExpandLT && (smx11 == 1 || SgInfo->Centric))
  {
    ...
    switch (SgInfo->LatticeInfo->Code)
    {
      case 'P':
        ...
        { ExpLT = LI_T; break; }
        if (IsSMxTransl0(LI_H, SMx->s.T) == 1)
        { ExpLT = LI_H; break; }
        if (IsSMxTransl0(LI_K, SMx->s.T) == 1)
        { ExpLT = LI_K; break; }
        if (IsSMxTransl0(LI_L, SMx->s.T) == 1)
        { ExpLT = LI_L; break; }
      case 'A':
      case 'B':
      case 'C':
        ...
        for (iLoopInv = 0; iLoopInv < nLoopInv; iLoopInv++)
        {
          ...
          if (i == 9) /* matrix has been found */
          {
            ...
            {                i = 0;
              if (Order < 0) i++;
              if (iLoopInv)  i += 2;
              (void) memcpy(ShTrMx, txrmx->ShTrMx[i], (sizeof (*ShTrMx)) * 9);
                                      ...
                                      T_RotMxInfo *BufRotMxInfo)
{
  ...
  else
  {
    ...
    if (GetRotMxInfo(SgInfo->ListSeitzMx[iList].s.R, RMxI, NULL) == 0) {
    ...
    while (iList < SgInfo->nList)
    {
      ...
      if (deter == -1 && SgInfo->Centric == -1)
      {
        ...
        if (i == SgInfo->nList)
        {
          ...
          if (   deter != 1
              || (lrmxi && GetRotMxInfo(lsmx->s.R, lrmxi, NULL) == 0)) {
            SetSgError(Err_Ill_SMx_in_List);
            return -1;
          }
        }
...
static int SgInfoListSortFunction(const int *iList_a, const int *iList_b)
{
  ...
  if (SgInfo->ListRotMxInfo == NULL)
  {
    ...
    if (   GetRotMxInfo(smx_a->s.R, rmxi_a, NULL) == 0
        || GetRotMxInfo(smx_b->s.R, rmxi_b, NULL) == 0)
    {
      SetSgError(Err_Ill_SMx_in_List);
      return 0;
    }
  ...
  if (val_a != 1)
  {
    ...
    if (rmxi_a->Order < rmxi_b->Order) return  1;
      ...
      i = CmpEigenVectors(rmxi_a->EigenVector, rmxi_b->EigenVector);
  if (i != 0)
    return i;
                ...
                int Order, int HonorSign, int RefAxis, int DirCode)
{
  ...
  for (iList = 0; iList < SgInfo->nList; iList++, lsmx++)
  {
    if (lrmxi == &RotMxInfo)
    {
      if (GetRotMxInfo(lsmx->s.R, lrmxi, NULL) == 0) {
        SetSgError(Err_Ill_SMx_in_List);
        return -1;
      }
    }
...
static int SetChiralFlag(T_SgInfo *SgInfo)
{
  int                iList;
  const T_RotMxInfo  *lrmxi;
  T_RotMxInfo        RotMxInfo;
  ...
  if (SgInfo->Centric || SgInfo->InversionOffOrigin) {
    SgInfo->Chiral = 0;
    return 0;
  }

  for (iList = 1; iList < SgInfo->nList; iList++) /* skip first          */
  {                                               /*   = identity matrix */
        lrmxi = ListOrBufRotMxInfo(SgInfo, iList, &RotMxInfo);
    if (lrmxi == NULL)
      return -1;
...
int CompleteSgInfo(T_SgInfo *SgInfo)
{
  ...
  switch (SgInfo->LatticeInfo->Code)
  {
    ...
    case 'T':
      ...
      break;
    case 'H': SgInfo->CCMx_LP = CCMx_HP; break;
    case 'K': SgInfo->CCMx_LP = CCMx_KP; break;
    case 'L': SgInfo->CCMx_LP = CCMx_LP; break;
    case 'F': SgInfo->CCMx_LP = CCMx_FP; break;
    default:
      break;
  }

  if (SetChiralFlag(SgInfo) != 0)
    return -1;
