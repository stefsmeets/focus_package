...
#define          MaxWtdExt  5
  ...
  *wl = '\0';
  ...
  if (WtdSgNumber == 0 && iwl == 0)
    return NULL;

  for (wl = WtdLbl; wl[0] && wl[1] && wl[2] && wl[3]; wl++)
  {
    if (isdigit(wl[0]) && wl[1] == '(' && isdigit(wl[2]) && wl[3] == ')')
    {
      wl[1] = wl[2];
      for (i = 2;; i++) {
        wl[i] = wl[i + 2];
...
static int Discretize(double fVal, int *iVal, int Fac)
{
  if (Fac == 0) return -1;
      fVal *= Fac;
...
#define P_Comma  0x10u
  ...
  do
  {
    if (! isspace(*StrXYZ)) switch (*StrXYZ)
    {
      case '_':
        break;
      ...
      case '8':
      case '9':
        if ((P_mode & P_Value) == 0) return -1;
        if (sscanf(StrXYZ, "%lf%n", &V, &i) != 1) return -1;
        StrXYZ += (i - 1);
  ...
  for (i = 0, Row = 0; Row < 3; Row++) {
    for (Column = 0; Column < 3; Column++, i++)
      if (RTMx->s.R[i]) break;
    ...
    if (LatticeInfo == NULL)
    {
      ...
      else if (c != ' ')
      {
        ...
        switch (c)
        {
          ...
          case 'H': LatticeInfo = LI_H; break;
          case 'K': LatticeInfo = LI_K; break;
          case 'L': LatticeInfo = LI_L; break;
...
static const char *PrintSgLabel(const char *lbl,
                                int space, int ParenthesizeSrew, int *n,
                                FILE *fpout)
{
  int  PrevDigit = 0;
  ...
  while (*lbl && *lbl != ' ')
  {
    ...
    else if (ParenthesizeSrew && isdigit(*lbl))
    {
      if (PrevDigit) {
        putc('(', fpout);
        putc(*lbl, fpout);
                       ...
                       int space, int ParenthesizeSrew, FILE *fpout)
{
  ...
  (void) PrintSgLabel(lbl, space, ParenthesizeSrew, &n, fpout);
                         ...
                         FILE *fpout)
{
  ...
  if (tsgn->SgNumber >= 3 && tsgn->SgNumber < 16)
  {
    lbl = PrintSgLabel(tsgn->SgLabels, space, ParenthesizeSrew, &n, fpout);
    ...
    n += PrintFullHM_SgName(tsgn, space, ParenthesizeSrew, fpout);
