                        ...
                        int *IniCBMxR, int *IniInvCBMxR)
{
  ...
  switch(SgInfo->XtalSystem)
  {
    ...
    case XS_Trigonal:
      ...
      else if (Code == 'H' || Code == 'K' || Code == 'L')
      {
        if      (NewPG == PG_321)  NewPG = PG_312;
        else if (NewPG == PG_312)  NewPG = PG_321;
        else if (NewPG == PG_3m1)  NewPG = PG_31m;
        else if (NewPG == PG_31m)  NewPG = PG_3m1;
        else if (NewPG == PG_3bm1) NewPG = PG_3b1m;
        else if (NewPG == PG_3b1m) NewPG = PG_3bm1;
      }
    ...
    case XS_Hexagonal:
      ...
      if (Code == 'H' || Code == 'K' || Code == 'L')
      {
        if      (NewPG == PG_6bm2) NewPG = PG_6b2m;
        else if (NewPG == PG_6b2m) NewPG = PG_6bm2;
      }
  ...
  iCoFactorMxTp(CCMx, IniInvCBMxR);
                           ...
                           const int **AlignRMx)
{
  ...
  if (GetRotMxInfo(UAMx, &RMxI_S, NULL) != RMxI_G->Order)
    goto ReturnError;
                   ...
                   int *FreeMx, int TryAgain)
{
  ...
  T_RTMx     SMxG, SMxS_G;
  int        NextFreeMxBuf[9], RmI_FMxBuf[9];
  int        RmI[9], *RmI_FMx, InvRmI_FMx[9], deterRmI_FMx;
  ...
  if (FreeMx) {
    RotMxMultiply(RmI_FMxBuf, RmI, FreeMx);
    RmI_FMx =     RmI_FMxBuf;
  }
  else
    RmI_FMx = RmI;
  ...
  for (;;)
  {
    deterRmI_FMx = FindInvertableMx(RmI_FMx, InvRmI_FMx,
    ...
    for (iTrV = 0; iTrV < nTrV; iTrV++, TrV += 3)
    {
      ...
      switch(nActive)
      {
        case 1:
          ...
          break;
        case 2:
          RedSh[icActive[0]] =   InvRmI_FMx[0] * S_G[irActive[0]]
                               + InvRmI_FMx[1] * S_G[irActive[1]];
          RedSh[icActive[1]] =   InvRmI_FMx[2] * S_G[irActive[0]]
                               + InvRmI_FMx[3] * S_G[irActive[1]];
          break;
        case 3:
          RotMx_t_Vector(RedSh, InvRmI_FMx, S_G, 0);
          break;
        default:
          break;
      }
      ...
      if (FreeMx)
      {
        ...
        for (i = 0; i < 3; i++)
          Sh[i] %= (CTBF * abs(deterRmI_FMx));
      }
      else
      {
        for (i = 0; i < 3; i++)
          Sh[i] = RedSh[i] % (CTBF * abs(deterRmI_FMx));
      }

      RotMx_t_Vector(CmpS_G, RmI, Sh, 0);

      for (i = 0; i < 3; i++)
        if ((CmpS_G[i] - S_G[i] * deterRmI_FMx) % (CTBF * abs(deterRmI_FMx)))
          break;
                        ...
                        T_RTMx       *CBMx,  T_RTMx       *InvCBMx)
{
  ...
  if (SgInfo->XtalSystem == XS_Monoclinic)
  {
    ...
    iCoFactorMxTp(RMxCCn_Buf, InvRMxCCn_Buf);
                                           ...
                                           T_RTMx *CBMx, T_RTMx *InvCBMx)
{
  ...
  for (tsgn = TabSgName; tsgn->HallSymbol; tsgn++)
  {
    ...
    (void) ParseHallSymbol(tsgn->HallSymbol, &GenSgI);
